...
export default function AppSidebar({ user, onLogout }: AppSidebarProps) {
  ...
  const learningItems: NavItem[] = user
    ? [
  ...
  const accountItems: NavItem[] = user ? [{ to: "/settings", label: "Настройки" }] : []

  return (
    <nav className="sticky top-0 flex h-screen w-[230px] shrink-0 flex-col overflow-y-auto border-r border-border bg-[#0c111a] px-3.5 py-[18px]">
