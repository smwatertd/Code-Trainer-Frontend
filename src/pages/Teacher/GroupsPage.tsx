...
export default function TeacherGroupsPage() {
  ...
  const openDashboard = (groupId: number) => {
    void navigate(`/teacher/groups/${groupId}/dashboard`)
  }
  ...
  const onInvite = async (event: MouseEvent, groupId: number) => {
    event.preventDefault()
            ...
            {groupsQuery.isLoading ? (
              ...
              groupsQuery.data.map((group) => (
                <div
                  ...
                  className="flex cursor-pointer flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-surface-2 p-3 transition hover:border-lime/40 hover:bg-surface"
                  onClick={() => openDashboard(group.id)}
                  onKeyDown={(event) => {
                    if (event.key === "Enter" || event.key === " ") {
                      event.preventDefault()
                      openDashboard(group.id)
                    }
                  }}
                >
                  <div className="min-w-0 flex-1">
                    <Link
                      ...
                      className="font-medium text-ink no-underline hover:text-lime"
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      onClick={(event) => void onInvite(event, group.id)}
